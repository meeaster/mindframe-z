...
- Assistant text + lifecycle (SessionStart/Stop/SessionEnd) ← the `event` bus (`session.next.text.ended`, `session.next.step.*`), **not** `chat.message` (whose `message` is the user message).
- PermissionRequest ← `permission.ask` `{status}`.
- `session_id` ← `sessionID` everywhere (carried on every hook/event).

### 6. Cost via msgpack `/v0.4/traces`, supplied verbatim
Hooks carry no cost; lapdog's pricing computation only runs on the parked proxy path. The validated path is a separate `POST /v0.4/traces` (msgpack) carrying an `_llmobs` envelope inside `meta_struct` as msgpack bytes; lapdog passes `meta_struct._llmobs.metrics` through verbatim into the synthesized LLMObs span. The host `mfz` process emits this after each dispatch using the usage it already parsed. **Token-split correctness:** the cost-span builder reads the raw Claude result `usage` (un-summed) so `non_cached_input_tokens`, `cache_read_input_tokens`, and `cache_write_input_tokens` are reported separately — not the collapsed `input_tokens` that `runs.json` stores. Cost fields are integer nanodollars: `estimated_total_cost`, `estimated_input_cost`, `estimated_output_cost`.

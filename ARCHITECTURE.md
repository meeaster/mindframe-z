...
A full `mfz apply` is the workspace activation command. It reconciles Executor when applicable, regenerates the local reference, extra-folder, and capability indexes, renders snapshots that embed those indexes, writes managed configuration, activates local files and links, and then synchronizes every enabled reference repository and removes deselected checkouts that MFZ owns and can delete safely. The indexes describe the resolved profile and do not depend on checkout revisions. Reference reconciliation also runs last for full-target `--agent` and `--no-link` invocations. Targeted Mise and dotfile applies remain scoped and do not synchronize references.

`mfz refs sync` reconciles all enabled references with bounded parallel checkout work and protected managed cleanup. The batch admits at most four checkouts, collects expected checkout failures while allowing later references to run, and stops admitting new work after a fatal error while joining every already-admitted checkout. `mfz refs sync <name>` acquires or updates only the named reference and preserves unrelated checkout and ownership state. Both forms use the same exclusive reference resource scope and regenerate local indexes but do not rerender agent snapshots or activate configuration. There is no separate reference-index command.

Reference updates and cleanup preserve local work. MFZ blocks destructive work when a checkout has modified or untracked files, ahead or divergent commits, an unexpected path or remote, or no matching ownership record. Ownership writes are serialized checkpoints, and a checkout or cleanup persistence failure is an honest operation outcome. Cleanup starts only after the selected-reference batch succeeds. A full apply may have already activated configuration when reference reconciliation fails; successful effects remain in place if a later operation fails.

Reference resource locks use canonical physical identities for the home state directory and checkout root, including the nearest existing physical ancestor when a path is missing. Lock paths are acquired in deterministic order and contention fails immediately. A cancellation joins admitted work before releasing the shared scope. An abandoned lock is not removed automatically: after verifying that no MFZ process is active, an operator may remove the reported lock directory manually.

Interactive reference lifecycle rows are keyed and updated on actual completion, while reference outcomes are buffered and emitted in deterministic manifest order rather than chronological verbose order. Retained rows show their final created, updated, removed, planned, blocked, or failed status; unchanged rows are removed. Captured output remains stable tab-separated text without terminal controls.

`mfz apply --dry-run` plans reference and index work from local state without cloning, fetching, pulling, deleting checkouts, or writing reference state and indexes. For an existing branch-tracking reference, the plan says that MFZ would check upstream because dry-run has no remote revision evidence.

Operation owners below the CLI classify actual outcomes. They compare bytes and managed modes, link destinations, skill digests, reference revisions, and external reconciliation results. The CLI-owned reporter in `src/cli/operation-report.ts` presents those outcomes. Default output lists meaningful changes and attention items, while `--verbose` adds unchanged and internal operations in emission order; reference outcomes are deliberately buffered and emitted in deterministic manifest order. Interactive terminals use one reporter-owned region for keyed reference lifecycle rows and ordinary operations. Captured output uses stable tab-separated plain lines without control sequences. A failed run reports completed effects and does not claim rollback.
...
Catalog entries use `source: local` or `source: vendored`. Local skills are authored in the home. A single-subtree vendored entry records an HTTPS repository, mutable tracked ref, and explicit upstream subtree; its selected files live under `skills/vendor/<name>/`. A provider-variant entry records the same repository and ref plus exactly one subtree for each of `claude-code`, `codex`, and `opencode`; its files live under `skills/vendor/<name>/<target>/`. `skills/vendor.lock.yml` records the full commit and aggregate framed SHA-256 digest, plus each provider digest for a variant. Symlinks, gitlinks, special files, submodules, LFS objects, hooks, dependencies, and candidate execution are outside the model.

Vendored updates fetch the tracked ref through a machine-local bare cache, stage an exact commit into quarantine, and require hostile-evidence review and explicit promotion before the home source and lock change. The committed vendored snapshot is the only upstream skill source rendered into a profile; harnesses never link directly to the cache.

Vendored authoring resolves catalog and profile declarations separately from activation-ready profiles. This lets `mfz skills stage` accept a new `source: vendored` declaration with no lock while `mfz apply`, rendering, and synchronization continue to require validated promoted source and lock state. The authoring result carries declaration ownership and profile selection but never trusted vendor provenance and cannot be passed to renderers.

For an unpromoted declaration, `mfz skills check [name]` reports the missing baseline without fetching. For a promoted skill, check validates local source and lock integrity, fetches only into a bare machine-local cache, and reports selected-subtree or provider-subtree changes. `mfz skills stage` extracts an exact revision into quarantine with provenance, inventory, findings, digest, and diff; candidate identity binds the complete target and trusted baseline state, while variant provenance includes the complete target-to-subtree/digest set. The stage output routes the reviewer to `mfz guide skill-review`, whose version-local procedure treats candidate text as hostile evidence and never executes it. `mfz skills promote` revalidates the candidate, rejects a changed trusted baseline, asks for explicit human confirmation, and atomically updates home source plus lock without applying. A later `mfz apply` activates the committed source.

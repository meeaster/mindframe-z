...
`mfz refs sync` reconciles all enabled references and protected managed cleanup. `mfz refs sync <name>` acquires or updates only the named reference and preserves unrelated checkout and ownership state. Both forms regenerate local indexes but do not rerender agent snapshots or activate configuration. There is no separate reference-index command.

Reference updates and cleanup preserve local work. MFZ blocks destructive work when a checkout has modified or untracked files, ahead or divergent commits, an unexpected path or remote, or no matching ownership record. Successful effects remain in place if a later operation fails.
...
Operation owners below the CLI classify actual outcomes. They compare bytes and managed modes, link destinations, skill digests, reference revisions, and external reconciliation results. The CLI-owned reporter in `src/cli/operation-report.ts` presents those outcomes. Default output lists meaningful changes and attention items, while `--verbose` adds chronological unchanged and internal operations. Interactive terminals use Clack with one current-operation indicator. Captured output uses stable tab-separated plain lines without control sequences. A failed run reports completed effects and does not claim rollback.

...
A full `mfz apply` is the workspace activation command. It synchronizes every enabled reference repository, removes deselected checkouts that MFZ owns and can delete safely, regenerates the local reference, extra-folder, and capability indexes, renders snapshots that embed those indexes, reconciles Executor when applicable, writes managed configuration, and activates local files and links. Reference reconciliation also runs for full-target `--agent` and `--no-link` invocations. Targeted Mise and dotfile applies remain scoped and do not synchronize references.
...
Reference updates and cleanup preserve local work. MFZ blocks destructive work when a checkout has modified or untracked files, ahead or divergent commits, an unexpected path or remote, or no matching ownership record. Ownership writes are serialized checkpoints, and a checkout or cleanup persistence failure is an honest operation outcome. Cleanup starts only after the selected-reference batch succeeds; local indexes and full-apply activation start only after reference reconciliation and cleanup succeed. Successful effects remain in place if a later operation fails.
